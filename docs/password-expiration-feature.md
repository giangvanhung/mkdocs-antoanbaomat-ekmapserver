...
```mermaid
stateDiagram-v2
    [*] --> Disabled: PasswordExpirationDays &lt;= 0
    [*] --> Valid: tuổi &lt; ReminderDays
    [*] --> ExpiringSoon: ReminderDays &lt;= tuổi &lt; ExpirationDays
    [*] --> Expired: tuổi &gt;= ExpirationDays
...
public async Task<PasswordExpirationStatus> GetStatusAsync(User user)
{
    var expirationDays = await _settingManager.GetSettingValueForUserAsync<int>(
        AppSettings.PasswordExpirationDays, user.TenantId, user.Id);

    var changeReminderDays = await _settingManager.GetSettingValueForUserAsync<int>(
        AppSettings.PasswordChangeReminderDays, user.TenantId, user.Id);
...
### 3.3. Ràng buộc `ReminderDays < ExpirationDays` khi lưu setting {#validation}
...
private void ValidatePasswordExpirationSettings(int passwordExpirationDays, int passwordChangeReminderDays)
{
    if (passwordExpirationDays < 0 || passwordChangeReminderDays < 0)
    {
        throw new UserFriendlyException(L("PasswordExpirationDaysCannotBeNegative"));
    }

    // Zero expiration turns the whole feature off, so the reminder value is irrelevant then.
    if (passwordExpirationDays > 0 && passwordChangeReminderDays >= passwordExpirationDays)
    {
...
Không đặt khoảng cách tối thiểu (kiểu "phải nhắc trước ít nhất 7 ngày"). Chỉ cần `reminder < expiration` là cấu hình đã có ý nghĩa. Nếu sau này chính sách bảo mật yêu cầu một khoảng nhắc tối thiểu, sửa điều kiện thành `expirationDays - passwordChangeReminderDays >= N` và đặt `N` thành hằng số trong `AppSettings` để cả server lẫn form Angular dùng chung một nguồn.
...
Điểm mấu chốt: từ khi có `LastPasswordChangeTime`, câu *"còn mấy ngày nữa hết hạn"* là một **hàm** của `(LastPasswordChangeTime, PasswordExpirationDays, hôm nay)`. Nó không phải một **sự kiện** xảy ra lúc login để phải ghi lại — nó là thứ tính ra được bất cứ lúc nào.
...
!!! danger "Audit log chỉ ghi giá trị mới, không ghi giá trị cũ"
    `AbpAuditLogs` lưu request gửi lên, nên nó trả lời được *"admin đặt `PasswordExpirationDays` = 0"* nhưng không trả lời được *"trước đó nó là 90"* — muốn biết phải tự dò ngược bản ghi trước đó.
...
!!! warning "App `Manager` chưa được cắm cảnh báo"
    Có **hai** app Angular, mỗi app một `topbar.component.ts` riêng, và cả hai đều đã gọi sẵn `getCurrentLoginInformations()`:
    ...
    Sau khi đăng nhập, `AccountController.GetAppHomeUrl()` trả về `Url.Action("Index", "Manager")` — người dùng đáp xuống app **Manager**. Nên hiện tại họ sẽ **không thấy cảnh báo** trừ khi tự chuyển sang khu Administrator.

    Backend không sai (`getCurrentLoginInformations()` trả `passwordExpiration` cho cả hai app), chỉ là phía client mới cắm vào một chỗ. Cần thêm đoạn tương tự vào topbar của `Manager`.
...
| `Configuration/AppSettings.cs` | `App.PasswordExpirationDays`, `App.PasswordChangeReminderDays` |
...
| `Configuration/Tenants/Dto/SecuritySettingsEditDto.cs` | `PasswordExpirationDays`, `PasswordChangeReminderDays` |
...
| `Manager/.../topbar/topbar.component.ts` | **Chưa làm** — xem cảnh báo ở mục 4.3 |
...
Sau migration, toàn bộ user hiện có sẽ có `LastPasswordChangeTime = NULL` → fallback về `CreationTime`. Nghĩa là **mọi user được tạo cách đây hơn `PasswordExpirationDays` ngày sẽ bị ép đổi mật khẩu ngay ở lần đăng nhập kế tiếp** — kể cả người vừa đổi mật khẩu tuần trước.
...
Xác nhận `App.PasswordExpirationDays` và `App.PasswordChangeReminderDays` trong `AbpSettings` có giá trị hợp lý. Không có row nào thì hệ thống dùng mặc định **90** và **80** (khai báo trong `AppSettingProvider`).
...
- Đặt `PasswordExpirationDays = 0`, giữ `PasswordChangeReminderDays = 80` → tính năng phải **tắt hoàn toàn**, không ai bị ép đổi. (Đây chính là kịch bản từng gây lockout toàn hệ thống — xem 3.2.)
- Đặt `PasswordChangeReminderDays = 90` với `PasswordExpirationDays = 90` → form phải **báo lỗi**, không lưu setting nào.
- Sửa thẳng `AbpSettings` bằng SQL cho `reminder = 95, expiration = 90` → hệ thống không được crash, chỉ đơn giản là không cảnh báo.
- Đặt `PasswordChangeReminderDays = 0` với `PasswordExpirationDays = 90` → vẫn hết hạn ở ngày 90, nhưng không có cảnh báo nào trước đó.
